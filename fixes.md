...
- [x] **Harden markdown rendering (security P1)**  
  ...
  - Add an iframe sandbox on the frontend (`frontend/src/main.js` preview element) with a minimal allowlist (kept `allow-same-origin` to preserve TOC scrolling) and set a strict CSP for the preview document (e.g., block scripts/remote loads).  
  - Add regression coverage that verifies script tags and inline event handlers are stripped/neutralized.

- [x] **Robust auto-reload watcher (robustness P1)**  
...
- [x] **TOC ID collision handling (usability P2)**  
...
- [x] **Large file safety (robustness P2)**  
